...
import Boton from "../Search/ButtonForm/ButtonForm";
import Input from "../Search/Input";

const Modal = ({ title, className="" }) => {

  const handleClick=(e)=>{
    document.querySelector("#has-hidden").classList.add("hidden")
  }

  return (
    <div id="has-hidden" className={className}>
      <div className="p-3 ">
        <button
      ...
      <div
        id="modal_overlay"
        className="fixed inset-0 bg-black bg-opacity-30 h-full w-full flex justify-center items-start md:items-center pt-10 md:pt-0"
      >
        <div
          id="modal"
          className="transform relative md:h-5/6 bg-white rounded shadow-lg transition-opacity duration-300"
        >
          <button onClick={handleClick} className="absolute -top-3 -right-3 bg-red-500 hover:bg-red-600 text-2xl w-10 h-10 rounded-full focus:outline-none text-white">
            X
          </button>
